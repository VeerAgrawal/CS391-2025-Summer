#Assignment 2, Mysolution02.py Final Sumbission file
#Veer Agrawal, veer1@bu.edu

#CODE Built from Prof solution 
...
chnot = term_lam("b", term_app(term_app(term_var("b"), chfls), chtru))
chand = term_lam("a", term_lam("b", term_app(term_app(term_var("a"), term_var("b")), chfls)))
chor = term_lam("a", term_lam("b", term_app(term_app(term_var("a"), chtru), term_var("b"))))

# CHURCH ISZERO CHECK
chnum_iszero = term_lam("n", term_app(
    term_app(term_var("n"), term_lam("x", chfls)),
    chtru
))


# Church helpers
chnum_lt = term_lam("m", term_lam("n", term_app(chnum_gtz, term_app(term_app(term_var("n"), chnum_pre), term_var("m")))))
chnum_sub = term_lam("m", term_lam("n", term_app(term_app(term_var("n"), chnum_pre), term_var("m"))))

chnum_div = term_fix_("f", "m",
    term_lam("n",
        term_if0_(
            term_app(term_app(chnum_lt, term_var("m")), term_var("n")),
            CHNUM0,
            term_app(
                chnum_suc,
                term_app(
                    term_app(term_var("f"),
                             term_app(term_app(chnum_sub, term_var("m")), term_var("n"))),
                    term_var("n")
                )
            )
        )
...
chnum_lte = term_lam("m", term_lam("n", term_app(chnum_iszero, term_app(term_app(term_var("n"), chnum_pre), term_var("m")))))
chnum_gt = term_lam("m", term_lam("n", term_app(chnum_gtz, term_app(term_app(term_var("m"), chnum_pre), term_var("n")))))
chnum_gte = term_lam("m", term_lam("n", term_app(chnum_iszero, term_app(term_app(term_var("m"), chnum_pre), term_var("n")))))
chnum_eq = term_lam("m", term_lam("n", term_app(term_app(chand, term_app(chnum_iszero, term_app(term_app(term_var("m"), chnum_pre), term_var("n")))), term_app(chnum_iszero, term_app(term_app(term_var("n"), chnum_pre), term_var("m"))))))
chnum_neq = term_lam("m", term_lam("n", term_app(chnot, term_app(term_app(chand, term_app(chnum_iszero, term_app(term_app(term_var("m"), chnum_pre), term_var("n")))), term_app(chnum_iszero, term_app(term_app(term_var("n"), chnum_pre), term_var("m")))))))
...
def term_comp00(tm0):
    ...
    if tm0.ctag == "TMopr":
        ...
        args = tm0.arg2
        assert len(args) == 2
        a1 = term_comp00(args[0])
        a2 = term_comp00(args[1])
        if opr == "+":
            return term_app(term_app(chnum_add, a1), a2)
        if opr == "-":
            return term_app(term_app(chnum_sub, a1), a2)
        if opr == "*":
            return term_app(term_app(chnum_mul, a1), a2)
        if opr == "/":
            return term_app(term_app(chnum_div, a1), a2)
        if opr == "%":
            return term_app(term_app(chnum_mod, a1), a2)
        if opr == "<":
            return term_app(term_app(chnum_lt, a1), a2)
        if opr == "<=":
            return term_app(term_app(chnum_lte, a1), a2)
        if opr == ">":
            return term_app(term_app(chnum_gt, a1), a2)
        if opr == ">=":
            return term_app(term_app(chnum_gte, a1), a2)
        if opr == "=":
            return term_app(term_app(chnum_eq, a1), a2)
        if opr == "!=":
            return term_app(term_app(chnum_neq, a1), a2)
        raise Exception("Unknown operator: " + opr)
