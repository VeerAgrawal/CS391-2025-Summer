##################################################################
# CODE COPIED FROM lecture-06-03/lambda3.py
# My Code is Marked by comments 
# Did some aditional debuging on prof's code to make everything run

import sys

#increased recursion limit:
# otherwise run into infinite recursion depth error 
...
######################## MY CODE TASK 1 
    ...
    # Compile a tuple (t1, t2)
    if (tm0.ctag == "TMtup"):
        cmp1 = term_comp01(tm0.arg1, cenv)
        cmp2 = term_comp01(tm0.arg2, cenv)
        inss = cmp1.arg1 + cmp2.arg1
        ttmp = ttmp_new()
        inss.append(tins_opr(ttmp, "tup", [cmp1.arg2, cmp2.arg2]))
        return tcmp(inss, ttmp)
    # Compile first projection expression (fst t)
    if (tm0.ctag == "TMfst"):
        cmp1 = term_comp01(tm0.arg1, cenv)
        ttmp = ttmp_new()
        inss = cmp1.arg1 + [tins_opr(ttmp, "fst", [cmp1.arg2])]
        return tcmp(inss, ttmp)
    # Compile second projection expression (snd t)
    if (tm0.ctag == "TMsnd"):
        cmp1 = term_comp01(tm0.arg1, cenv)
        ttmp = ttmp_new()
        inss = cmp1.arg1 + [tins_opr(ttmp, "snd", [cmp1.arg2])]
        return tcmp(inss, ttmp)
    # Compile let binding expression (let x = t1 in t2)
    if (tm0.ctag == "TMlet"):
        cmp1 = term_comp01(tm0.arg2, cenv)
        tmp1 = cmp1.arg2
        cenv1 = cenv_cons(tm0.arg1, tmp1, cenv)
        cmp2 = term_comp01(tm0.arg3, cenv1)
        inss = cmp1.arg1 + cmp2.arg1
        return tcmp(inss, cmp2.arg2)
################# END OF MY CODE task 1
    if (tm0.ctag == "TMapp"):    
        cmp1 = term_comp01(tm0.arg1, cenv)
        cmp2 = term_comp01(tm0.arg2, cenv)
        ins1 = cmp1.arg1
    ...
    if (tm0.ctag == "TMlam"):
        ...
        body = tm0.arg3 if hasattr(tm0, "arg3") else tm0.arg2
        cmp1 = term_comp01(body, cenv)
    ...
    if (tm0.ctag == "TMfix"):
        ...
        body = tm0.arg5 if hasattr(tm0, "arg5") else tm0.arg3
        cmp1 = term_comp01(body, cenv)
        inss = [tins_fun(fun0, cmp1, arg0)]
        return tcmp(inss, fun0)
...
# TASK 2: implement function tcmp_pyemit

def treg_py(treg0):
    return treg0.prfx + str(treg0.sffx)

def tval_py(tv0):
    if tv0.ctag == "TVALint":
        return str(tv0.arg1)
    if tv0.ctag == "TVALbtf":
        return "True" if tv0.arg1 else "False"
    raise TypeError(tv0)

def tcmp_pyemit_cmp(cmp0, indent):
    lines = []
    ind = " " * indent
    for ins in cmp0.arg1:
        if ins.ctag == "TINSmov":
            dst = treg_py(ins.arg1)
            src = tval_py(ins.arg2)
            lines.append(f"{ind}{dst} = {src}")
        elif ins.ctag == "TINSopr":
            dst = treg_py(ins.arg1)
            opr = ins.arg2
            regs = [treg_py(r) for r in ins.arg3]
            if opr == "tup":
                expr = f"({regs[0]}, {regs[1]})"
            elif opr == "fst":
                expr = f"{regs[0]}[0]"
            elif opr == "snd":
                expr = f"{regs[0]}[1]"
            elif opr == "cmp":
                expr = f"(-1 if {regs[0]} < {regs[1]} else (1 if {regs[0]} > {regs[1]} else 0))"
            else:
                pyop = opr if opr != "=" else "=="
                expr = f"{regs[0]} {pyop} {regs[1]}"
            lines.append(f"{ind}{dst} = {expr}")
        elif ins.ctag == "TINSapp":
            dst = treg_py(ins.arg1)
            fun = treg_py(ins.arg2)
            arg = treg_py(ins.arg3)
            lines.append(f"{ind}{dst} = {fun}({arg})")
        elif ins.ctag == "TINSfun":
            fname = treg_py(ins.arg1)
            param = treg_py(ins.arg3)
            lines.append(f"{ind}def {fname}({param}):")
            blines, bres = tcmp_pyemit_cmp(ins.arg2, indent + 4)
            lines.extend(blines)
            lines.append(f"{' '*(indent+4)}return {bres}")
        elif ins.ctag == "TINSif0":
            dst = treg_py(ins.arg1)
            test = treg_py(ins.arg2)
            lines.append(f"{ind}if {test}:")
            tlines, tres = tcmp_pyemit_cmp(ins.arg3, indent + 4)
            lines.extend(tlines)
            lines.append(f"{' '*(indent+4)}{dst} = {tres}")
            lines.append(f"{ind}else:")
            elines, eres = tcmp_pyemit_cmp(ins.arg4, indent + 4)
            lines.extend(elines)
            lines.append(f"{' '*(indent+4)}{dst} = {eres}")
        else:
            raise TypeError(ins)
    res = treg_py(cmp0.arg2)
    return lines, res

def tcmp_pyemit(cmp0, fname="prog"):
    lines, res = tcmp_pyemit_cmp(cmp0, 4)
    code = [f"def {fname}():"] + lines + [f"    return {res}"]
    return "\n".join(code)

#############################END OF MY CODE TASK 2 

# -----------------------------------------
# TESTING !!!
# -----------------------------------------

# I COPIED OVER MIDTERM SOLUTION FILE (MIDTERM.PY) TO THIS PATH 
# SO THAT I CAN IMPORT AND TEST MY QUEENS EIGHT PUZZLE SOLUTION

from midterm import search, term_app, term_int, term_lam


print("\n=== COMPILING 8-QUEEN LAMBDA TERM ===")

# Use the same 8-queen LAMBDA term from midterm
const0 = term_lam("_", term_int(0))

# search(bd=const0)(i=0)(j=0)(nsol=0)
term_start = term_app(
    term_app(
        term_app(
...
# Compile the term to tcmp
compiled = term_comp00(term_start)
print("\n=== COMPILED STACK-IR PROGRAM (tcmp) ===")
print(compiled)

# Emit Python code from tcmp
print("\n=== GENERATED PYTHON CODE ===")
py_code = tcmp_pyemit(compiled, fname="eight_queen_solver")
print(py_code)

# Optional: run the emitted code and check result
print("\n=== EXECUTING GENERATED CODE ===")
namespace = {}
exec(py_code, namespace)
result = namespace["eight_queen_solver"]()
print(f"Generated solver output: {result}")
...
# Test 1: Simple binary operations
print("\n--- Test: Binary Operators ---")
ops = ["+", "-", "*", "<", "<=", ">", ">=", "=", "!=", "cmp"]
for op in ops:
    term = term_opr(op, [int_1, int_2])
    compiled = term_comp00(term)
    print(f"comp00({op}) = {compiled}")

# Test 2: Tuple creation and projection
print("\n--- Test: Tuple Construction and Projections ---")
tup = term_tup(int_1, int_2)
fst = term_fst(tup)
snd = term_snd(tup)
print("comp00(term_tup(1,2)) =", term_comp00(tup))
print("comp00(term_fst(tup)) =", term_comp00(fst))
print("comp00(term_snd(tup)) =", term_comp00(snd))

# Test 3: Let-binding
print("\n--- Test: Let Binding ---")
let_expr = term_let("x", term_int(42), term_var("x"))
print("comp00(let x = 42 in x) =", term_comp00(let_expr))

# Test 4: Lambda + application
print("\n--- Test: Lambda and Application ---")
lam = term_lam("x", term_opr("+", [term_var("x"), int_1]))
app = term_app(lam, int_2)
print("comp00((λx. x+1) 2) =", term_comp00(app))

# Test 5: Nested let + tuple
print("\n--- Test: Nested Let with Tuple ---")
nested_let = term_let("p", term_tup(term_int(4), term_int(5)),
               term_let("x", term_fst(term_var("p")),
               term_let("y", term_snd(term_var("p")),
                        term_opr("+", [term_var("x"), term_var("y")])
               )))
print("comp00(nested tuple/let add) =", term_comp00(nested_let))
